...
taskUpdate=async()=>{
    ...
    xhr.addEventListener("readystatechange", function() {
        if (xhr.readyState !== 4) {
          ...
          _this.hideLoader();
          _this.enableModal(xhr.status, "059");
        }
    });
...
value=async()=>{
  ...
  back(){
    ...
    //context.props.navigation.navigate("CreateTaskWithMyTasks");
   }
  render (){
    const context=this;
  ...
  return (
    ...
    <View style={{left:'12%',top:'36%'}}>
     <Text style={{color:'white',fontSize:20,fontWeight:'bold'}}>Task Update</Text>
      </View>
     ...
     {this.state.update_comment.map((item) => {
         return(
                 <View style={{flexDirection:'row'}}>
                     <View>
                     <Image source={this.state.image} style={{left:'5%',bottom:0,height:40,width:40,borderRadius:75,borderColor:'black',alignItems:'center',borderColor:'transparent',borderWidth:1,top:'25%'}}/>
                     </View>
                     <View style={{width:'80%',left:'10%',top:'5%'}}> 
                     <View style={{flexDirection:'row'}}>
                      <Text style={{color:'rgb(19,111,232)'}}>{this.state.name}</Text>
