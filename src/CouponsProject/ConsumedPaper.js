...
import {View, Text, TextInput, StyleSheet, Platform, FlatList, Keyboard, ActivityIndicator, Alert} from 'react-native';
...
class ConsumedPaper extends Component{
    constructor(props){
        super(props)
        this.state = {
            fromDate: '',
    ...
    fetchReport(){
        ...
        this.showLoader();
        axios.post(`${baseURL}/customs/paper-consumption-report`,
        {
            date_from: fromDate, 
            date_to: toDate, 
        ...
        },
        {
            ...
            this.setState({paperData: responseJson.success.paperOutObj}, () => {})
        }).catch((error) => {
            this.hideLoader();
    ...
    render(){
        const {fromDate, fromDateError, toDate, toDateError, submit, loading, paperData, portName} = this.state;
                    ...
                    <View style={styles.container}>
                        <View style={[{
                            ...
                            <View style={[{alignSelf: 'flex-start'}, getMarginTop(2)]}>
                                <Text style={[{color: colorTitle}, fontSizeH4(), getMarginLeft(5)]}>TOTAL PAPERS CONSUMED</Text>
                            </View>
                            <View style={[getMarginTop(1)]}>
                                <View style={[{backgroundColor: colorTitle, flexDirection: 'row'}, getWidthnHeight(84, 5)]}>
                                    {tableHeader.map((item, index) => {
                                        return (
                                            <View style={[{alignItems: 'center', justifyContent: 'center', borderColor: 'white', borderRightWidth: 0}, getWidthnHeight(28, 5)]}>
                            ...
                            <View style={[{flex: 1}, getMarginBottom(2)]}>
                                {(paperData.length > 0)?
                                        ...
                                        renderItem={({item, index}) => {
                                            return (
                                                <View style={[{flexDirection: 'row', alignItems: 'center', borderLeftWidth: 1, borderRightWidth: 1, borderBottomWidth: 1, borderColor: 'rgba(196, 196, 196, 0.3)'}, getWidthnHeight(84, 5)]}>
                                                    <View style={[{alignItems: 'center', justifyContent: 'center', borderWidth: 0, borderColor: 'red'}, getWidthnHeight(28, 7)]}>
                                                        <Text style={[{color: '#000000', fontSize: (fontSizeH4().fontSize + 2)}]}>{item.paper_type}</Text>
                                                    </View>
                                                    <View style={[{alignItems: 'center', justifyContent: 'center'}, getWidthnHeight(28, 5)]}>
                                                        <Text>{item.total_utilized}</Text>
                                                    </View>
                                                    <View style={[{alignItems: 'center', justifyContent: 'center'}, getWidthnHeight(28, 5)]}>
                                                        <Text>{item.total_wasted}</Text>
                                                    </View>
                                                </View>
                                            );
                                        }}
                                    />
                                :
                                    <View style={[{alignItems: 'center', justifyContent: 'center', borderColor: '#C4C4C4', borderWidth: (loading || paperData.length === 0)? 0.5 : 0}, getWidthnHeight(84, 5)]}>
                                        <Text style={{fontSize: (fontSizeH4().fontSize + 2)}}>{(loading)? 'Please wait...' : (paperData.length === 0)? 'No records were found' : 'Please wait...'}</Text>
                                    </View>
                                }
                            </View>
                        </View>
