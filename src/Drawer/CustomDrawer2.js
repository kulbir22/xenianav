...
function CustomDrawer(props) {
    ...
    const recallDrawerUpdate = useCallback(() => {
        const routes = props.navigation.getState().routes;
        __DEV__ && console.log("$$SSDD##@@ DRAWER ROUTES: ", routes)
                ...
                item.subcategory.forEach((subItem) => {
                    routes.forEach((routeItem) => {
                        if(subItem.type === routeItem.name){
                            subCategory.push({
                                id: 'MainDrawer', routeName: subItem.type, title: subItem.val,
                                key: routeItem.key
                            })
                        }
                    })
        ...
        home = Object.assign(home, { title: home.name, routes: [{id: 'MainDrawer', routeName: home.name, title: home.name}] })
    ...
    function selectSubCategory(parentKey, selectedItem){
        ...
        setNewDrawer(updateDrawer);
        props.navigation.toggleDrawer();
        props.navigation.navigate(selectedItem.routeName);
    }
